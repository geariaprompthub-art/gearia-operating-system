...
class FakeEmbeddingProvider:
    def __init__(self, fail: bool = False, dimensions: int = EMBEDDING_DIMENSIONS, fail_indices: set[int] | None = None) -> None:
        self.calls = 0; self.fail = fail; self.dimensions = dimensions; self.fail_indices=fail_indices or set(); self.inputs: list[str]=[]; self.failed_indices: list[int]=[]
    ...
    def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        for text in texts:
            ...
            vectors.append([((sha256(f"{text}:{value}".encode()).digest()[0] / 255) * 2 - 1) for value in range(self.dimensions)])
        return vectors
